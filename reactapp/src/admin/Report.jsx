...
const Report = () => {
  ...
  const checkAuth = () => {
    ...
    if (!token) {
      toast.error("Please login to access report management features.");
      return false;
    }
    return true;
  };
  ...
  const createReport = async () => {
    ...
    if (!formData.type || !formData.generatedBy || !formData.format) {
      toast.error("Please fill in required fields: Type, Generated By, and Format");
      return;
    }
    ...
    try {
      const res = await axios.post(API_BASE, formData, { headers });
      ...
      if (err.response?.status === 403) {
        ...
        toast.error("Error creating report: " + (err.response?.data || err.message));
      }
    }
  };
  ...
  const updateReport = async () => {
    ...
    if (!editingReport || !formData.type || !formData.generatedBy || !formData.format) {
      toast.error("Please fill in required fields: Type, Generated By, and Format");
      return;
    }
    ...
    try {
      const res = await axios.put(`${API_BASE}/${editingReport.reportId}`, formData, { headers });
      ...
      if (err.response?.status === 403) {
        ...
        toast.error("Error updating report: " + (err.response?.data || err.message));
      }
    }
  };
  ...
  const deleteReport = async (reportId) => {
    ...
    try {
      const res = await axios.delete(`${API_BASE}/${reportId}`, { headers });
      ...
      if (err.response?.status === 403) {
        ...
        toast.error("Error deleting report: " + (err.response?.data || err.message));
      }
    }
  };
  ...
  return (
    <div style={styles.container}>
      <div style={styles.card}>
        <h1 style={styles.heading}>📊 Report Management System</h1>
        ...
        <div style={styles.chartContainer}>
          <h3 style={{textAlign: "center", color: "#1e40af", marginBottom: "20px"}}>📈 Report Format Distribution</h3>
