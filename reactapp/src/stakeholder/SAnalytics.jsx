...
const SAnalytics = () => {
  ...
  const checkAuth = () => {
    ...
    if (!token) {
      toast.error("Please login to access analytics features.");
      return false;
    }
    return true;
  };
  ...
  const fetchReportsByType = async () => {
    ...
    try {
      const res = await axios.get(`${API_BASE}/type/${selectedType}`, { headers });
      setReports(res.data);
      toast.success(`Fetched reports for type ${selectedType} successfully`);
    } catch (err) {
      if (err.response?.status === 403) {
        toast.error("Access denied. Please login with proper permissions.");
      } else {
        toast.error("Error fetching reports by type: " + (err.response?.data || err.message));
      }
    }
  };

  // READ - Get reports by creator
  const fetchReportsByCreator = async () => {
    if (!checkAuth()) return;
    ...
    try {
      const res = await axios.get(`${API_BASE}/creator/${selectedCreator}`, { headers });
      setReports(res.data);
      toast.success(`Fetched reports for creator ${selectedCreator} successfully`);
    } catch (err) {
      if (err.response?.status === 403) {
        toast.error("Access denied. Please login with proper permissions.");
      } else {
        toast.error("Error fetching reports by creator: " + (err.response?.data || err.message));
      }
    }
  };
  ...
  return (
    <div style={styles.container}>
      <div style={styles.card}>
        <h1 style={styles.heading}>📊 Analytics Report Viewer (Stakeholder)</h1>
        ...
        <div style={styles.form}>
          ...
          <input
            type="text"
            placeholder="Report Type (for GET by Type)"
            value={selectedType}
          ...
          <input
            type="text"
            placeholder="Creator Name (for GET by Creator)"
        ...
        <div style={styles.buttonContainer}>
          ...
          <button 
            style={{...styles.button, ...styles.readBtn}} 
            onClick={fetchReportsByType}
          >
            📊 GET by Type
          </button>
          <button 
            style={{...styles.button, ...styles.readBtn}} 
            onClick={fetchReportsByCreator}
          >
            👤 GET by Creator
          </button>
        </div>
      ...
      {/* Table */}
      <div style={styles.card}>
        <h3>📊 Reports List ({reports.length} total)</h3>
