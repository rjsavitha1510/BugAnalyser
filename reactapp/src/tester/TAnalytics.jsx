...
const TAnalytics = () => {
  ...
  const checkAuth = () => {
    ...
    if (!token) {
      setError("Please login to access analytics management features.");
      clearMessages();
      return false;
    }
    return true;
  ...
  const fetchAllReports = async () => {
    ...
    try {
      ...
      setSuccess(`Fetched ${res.data.length} reports successfully`);
      clearMessages();
    } catch (err) {
      if (err.response?.status === 403) {
        setError("Access denied. Please login with proper permissions.");
      } else {
        setError("Error fetching reports: " + (err.response?.data || err.message));
      }
      clearMessages();
    }
  };
  ...
  const fetchReportById = async () => {
    ...
    try {
      ...
      setReports([res.data]);
      setSuccess(`Report ${selectedReportId} fetched successfully`);
      clearMessages();
    } catch (err) {
      if (err.response?.status === 403) {
        setError("Access denied. Please login with proper permissions.");
      } else if (err.response?.status === 404) {
        setError(`Report with ID ${selectedReportId} not found`);
      } else {
        setError("Error fetching report: " + (err.response?.data || err.message));
      }
      clearMessages();
    }
  };
